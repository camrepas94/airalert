...
export type ActivityNotificationKind =
  ...
  | "community_reply"
  | "group_chat_invite"
  /** Watch party lobby reminder — opens Community → Watch Party. */
...
export function pushNotificationCategory(kind: PushNotificationKind): NotificationCategory {
  ...
  if (kind === "watchPartyReminder") return "live_room";
...
export function pushNotificationSurface(kind: PushNotificationKind): NotificationSurface {
  ...
  if (kind === "communityMention" || kind === "communityReply" || kind === "watchPartyReminder") return "activity_dropdown";
